...
class functions
{
    ...
    { //convert 31/12/2019 thang 2019-12-31
        if ($text != '') {
    ...
    {   
        //convert 0975222333 thang 0975.222.222
        $result = $phone;
    ...
    public static function textLimit($str, $limit)
    {
        $str_s = '';
        if (stripos($str, " ")) {
            $ex_str = explode(" ", $str);
    ...
    public static function generateActiveOrder($active)
    {
        switch ($active) {
            case '1':
                return '<span class="badge badge-secondary">Chưa đặt cọc</span>';
                break;
            default:
                return '<span class="badge badge-primary">Đã đặt cọc</span>';
                break;
        }
    }
}
