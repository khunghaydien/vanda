  ...
  function pageInfo($url)
  {
    ...
    if (isset($url[1]) && $url[1] == 'blog')
      if (sizeof($url) == 3) {
        ...
        // danh muc bai viet
        $query1 = $this->db->query("SELECT id, name, image, description FROM blogcategory WHERE status=1 AND url='" . $url[3] . "' ");
    ...
    elseif (isset($url[1]) && $url[1] == 'product')
      if (sizeof($url) == 3) {
        // chi tiet san pham
        $query = $this->db->query("SELECT *,
                ...
                (SELECT url FROM productcategory WHERE id= p.category) AS danhmucurl
                  FROM product p WHERE url='" . $url[2] . "'");
        if(!$query){
          $query = $this->db->query("SELECT *,
                (SELECT link from productpicture WHERE product = p.id ORDER BY position LIMIT 1 ) as image, 
                (SELECT name FROM productcategory WHERE id= p.category) AS danhmuc,
        ...
        $dem = $temp[0]['view'];
        $new = $dem + 1;
        $query1 = $this->db->query("UPDATE product set view=$new WHERE url='" . $url[2] . "' ");
        ...
        // danh muc san pham
        $query1 = $this->db->query("SELECT id, name, url, description, image FROM productcategory WHERE url='$url[3]' AND status= 1");
    ...
    else if (isset($url[1]) && $url[1] == 'search'){
      $query = $this->db->query("SELECT * FROM infomation");
      $temp = $query->fetchAll(PDO::FETCH_ASSOC);
