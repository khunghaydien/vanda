...
<div class="col-lg-12">
    <div class="card">
        <form action="sanpham/editsave?id=<?= $this->data['id'] ?>" method="post" enctype="multipart/form-data">
            <div class="card-header"><strong>Cập nhật</strong><small> Sản phẩm</small></div>
            <div class="card-body card-block">
                <div class="row form-group">
                    <div class="col-12 col-md-5">
                        <div class="form-group"><label for="city" class=" form-control-label">Tên</label><input type="text" id="name" name="name" placeholder="Nhập tên sản phẩm" class="form-control" value="<?= $this->data['name'] ?>" required="required"></div>
                    </div>
                    <div class="col-12 col-md-5">
                    ...
                    <div class="col-12 col-md-2">
                        <div class="form-group"><label for="postal-code" class=" form-control-label">Tình trạng</label>
                            <select name="status" class="form-control">
                                <option <?php if($this->data['status'] == 1) echo "selected"; ?> value="1">Hiển thị</option>
                                <option <?php if($this->data['status'] == 2) echo "selected"; ?> value="2">Ẩn</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="row form-group">
                    <div class="col-12 col-md-3">
                        <div class="form-group"><label for="city" class=" form-control-label">Model</label>
                            <input type="text" placeholder="Model" name="ma" value="<?= $this->data['code'] ?>" class="form-control">
                        </div>
                    </div>
                    <div class="col-12 col-md-3">
                        <div class="form-group"><label for="city" class=" form-control-label">Tag</label>
                            <input type="text" placeholder="Tag" name="tag" value="<?= $this->data['tag'] ?>" class="form-control">
                        </div>
                    </div>
                    <div class="col-12 col-md-3">
                        <div class="form-group"><label for="postal-code" class=" form-control-label">Url</label><input type="text" name="url" placeholder="Để trống sẽ lấy theo tên" value="<?= $this->data['url'] ?>" class="form-control"></div>
                    </div>
                    <div class="col-12 col-md-3">
                        <div class="form-group"><label for="postal-code" class=" form-control-label">Vị trí</label><input type="text" name="position" placeholder="Vị trí nổi bật" value="<?= $this->data['position'] ?>" class="form-control"></div>
                    </div>
                </div>
                        ...
                        <div class="w-100 col-12">
                            <?php foreach ($this->thuoctinh as $key =>  $value) { ?>
                                <div class="col-6 col-md-3">
                                    <div class="form-group"><label for="postal-code" class=" form-control-label"><?= $value['name'] ?></label><input type="text" placeholder="<?= $value['name'] ?>" name="thuoctinh<?= ($key + 1) ?>" value="<?= $this->thuoctinhvalue[$key]['value'] ?>" class="form-control"></div>
                                </div>
                            <?php } ?>
                        </div>
                ...
                <div class="row form-group">
                    <div class="col-12">
                        <div class="form-group"><label for="city" class=" form-control-label">Thông số kỹ thuật</label>
                            <textarea rows="10" class="form-control" name="mo_ta"><?= $this->data['description'] ?></textarea>
                        </div>
                    </div>
                </div>
                <div class="row form-group d-none">
                    <div class="col-12">
                        <div class="form-group"><label for="city" class=" form-control-label">Nội dung</label>
                            <textarea rows="10" class="form-control" name="noidung"><?= $this->data['content'] ?></textarea>
                        </div>
                    </div>
                </div>
                <div class="row form-group">
    ...
    <script id="jsNeedToCopy">
        ...
        var myImageUpload = new ImageUpload({
            ...
            saveImageRoute: "sanpham/saveimageedit",
            imageBelongsTo: {
        ...
        setTimeout(function() {
            <?php $dem = count($this->hinhanh);
            foreach ($this->hinhanh as $key => $value) { ?>
                var imageSrc = "<?= $value['link'] ?>";
                var imageName = '<?= $value['link'] ?>';
                var imageId = '<?= $value['id'] ?>';
                <?php if ($dem == ($key + 1)) {
                    echo 'myImageUpload.appendServerImage(imageSrc, imageName, imageId);
                    }, 500);';
                } else {
                    echo ' myImageUpload.appendServerImage(imageSrc, imageName, imageId);';
                } ?>
