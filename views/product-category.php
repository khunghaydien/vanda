<?php
if(isset($page['error']) && $page['error'] == true){
...
$productList = $data->getProductCategory($currentPage, $page['category']['id'], $sort, $limit);
    ...
    <div class="page-title-inner flex-row  medium-flex-wrap container">
        <div class="flex-col flex-grow medium-text-center">
            <div class="is-large">
                <nav class="woocommerce-breadcrumb breadcrumbs">
                    <a href="<?= HOME ?>">Trang chủ</a><span class="divider">&#47;</span> 
                    <a href="<?= HOME.'/product' ?>">Sản phẩm</a><span class="divider">&#47;</span> 
                    <?=$page["title"]?>
                </nav>
            </div>
        </div><!-- .flex-left -->
        <div class="flex-col medium-text-center">
            <p class="woocommerce-result-count hide-for-medium">
                Hiển thị <?= $from.'&ndash;'.$end ?> của <?=$total?> sản phẩm</p>
            <div class="woocommerce-ordering" method="get">
                <select onchange="changeSortProductCategory(this.value, <?=$currentPage?>, '<?= $page['category']['url'] ?>')" >
                    <option <?php if($sort == 0) echo 'selected'; ?> value="0" selected='selected'>Mới nhất</option>
                    <option <?php if($sort == 1) echo 'selected'; ?> value="1">Thứ tự theo tên A - Z</option>
                    <option <?php if($sort == 2) echo 'selected'; ?> value="2">Thứ tự theo tên Z - A</option>
                </select>
            </div>
        </div><!-- .flex-right -->
    </div><!-- flex-row -->
                                ...
                                <div class="product-small box ">
                                    ...
                                    <div class="box-text text-center">
                                        <div class="title-wrapper">
                                            <p class="name product-title"><a href="product/<?= $product['url'] ?>"><?= functions::textLimit($product['name'], 14) ?></a></p>
                                        </div>
                                        <div class="price-wrapper">
                                        </div>
                                        <div class="add-to-cart-button">
                                            <a href="product/<?= $product['url'] ?>" rel="nofollow" data-product_id="2437" class="ajax_add_to_cart  product_type_simple button primary is-outline mb-0 is-small">
